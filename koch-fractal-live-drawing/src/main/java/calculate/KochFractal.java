...
public class KochFractal implements Observable {
    ...
    private void drawKochEdge(double ax, double ay, double bx, double by, int n) {
        if (!cancelled) {
            if (n == 1) {
                ...
                calculatedEdge = new Edge(ax, ay, bx, by, Color.hsb(hue * 360.0, 1.0, 1.0));
                notifyObservers();
            } else {
